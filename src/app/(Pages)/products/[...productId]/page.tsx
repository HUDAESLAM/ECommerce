import { Params } from "next/dist/server/request/params";
import React from "react";
import {
  Card,
  CardContent,
  CardDescription,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import ProductSlider from "@/app/_Components/carasoul/ProductSlider";
import AddToCart from "@/app/_Components/AddToCart/AddToCart";
import { Product} from "@/interfaces";

export default async function ProductDetails({ params }: { params: Params }) {
  const { productId } =  params;


  const response = await fetch(
    `${process.env.URL_API}/products/${productId}`
  );
  const  {data : productData } : {data : Product}  = await response.json();

  console.log(productData);

  return (
    <>
      <Card className="grid md:grid-cols-3 items-center">
        <div className="col-span-1">
          <ProductSlider images={productData.images} altContact={productData.title}/>
        </div>
        <div className="md:col-span-2 space-y-4">
          <CardHeader>
            <CardDescription>{productData.brand.name}</CardDescription>
            <CardTitle className="text-xl">{productData.title}</CardTitle>
            <CardDescription>{productData.description}</CardDescription>
          </CardHeader>
          <CardContent>
            <CardDescription>{productData.category.name}</CardDescription>
            <div className="flex gap-6 items-center justify-between mt-3">
              <p className="flex gap-1">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                  className="size-6 text-yellow-500"
                >
                  <path
                    fillRule="evenodd"
                    d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.006 5.404.434c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.434 2.082-5.005Z"
                    clipRule="evenodd"
                  />
                </svg>
                <span>{productData.ratingsAverage}</span>
              </p>
              <p className="flex gap-1">
                <span>Remining :{productData.ratingsQuantity}</span>
              </p>
            </div>
            <div className="flex gap-6 items-center justify-between mt-3">
              <p className="flex gap-1">
                <span className="text-xl font-semibold">
                  Price: {productData.price}
                </span>
                EGP
              </p>
              <p className="flex gap-1">
                <span>Quantity :{productData.quantity}</span>
              </p>
            </div>
          </CardContent>
          <AddToCart productId={productData.id}/>
        </div>
      </Card>
    </>
  );
}
